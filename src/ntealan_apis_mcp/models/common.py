...
class ConfigModel(BaseModel, extra=Extra.forbid):
    ...
    api_log_enable: bool = Field(
        default=False,
        env="NTEALAN_API_LOG_ENABLE",
        description="Enable logging for API requests"
    )
    api_log_level: str = Field(
        default="info",
        env="NTEALAN_API_LOG_LEVEL",
        description="Log level for API requests"
    )
